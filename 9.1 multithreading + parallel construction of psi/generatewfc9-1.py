...
class WfcGenerator:
    ...
    def run(self):
        ...
        if isinstance(self.nk_points, range):
            ...
            with concurrent.futures.ThreadPoolExecutor(m.nks) as executor:
                executor.map(lambda nk: self._wfck2r(nk, 0, m.nbnd), self.nk_points)
    ...
    def _parallel_wfck2r(self, nk_point):
        self.logger.info(f"\tCalculating wfc for k-point {nk_point}")
        self._wfck2r(nk_point, 0, m.nbnd)
    ...
    def _wfck2r(self, nk_point: int, initial_band: int, number_of_bands: int):
        ...
        if m.noncolin:
            ...
            outfile = os.path.join(m.wfcdirectory, f"k0{nk_point}.npy") #savez doesn't like to save with extensions other than .npz

            with open(outfile, "wb") as fich:
                np.save(fich, psifinal)

        #    l = np.load(outfile)
         #   if not np.array_equal(l, psifinal):
          #      print(False, nk_point)
