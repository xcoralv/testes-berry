    ...
    def _get_command(self, nk_points: int, initial_band: int, number_of_bands: int):
        #with self.lock:
            ...
            if m.noncolin:
               ...
               # return f'echo "{command}" | {mpi} wfck2r.x | tail -{m.nr * number_of_bands * m.nks}'
               # return f'echo "{command}" | {mpi} wfck2r.x > /dev/null; tail -{m.nr * number_of_bands * m.nks} {m.wfck2r}'
