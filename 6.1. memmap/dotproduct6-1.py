...
def dot(nk: int, j: int, neighbor: int, jNeighbor: Tuple[np.ndarray]) -> None:
    ...
    if m.noncolin:  # Noncolinear case
        ...
        for band0 in range(m.nbnd):
            wfc0 = np.memmap(os.path.join(m.wfcdirectory, f"k0{nk}.wfc"), mode='r', dtype=np.complex128,
                            offset=band0 * m.nr * np.dtype(np.complex128).itemsize, shape=(m.nr,))

            for band1 in range(m.nbnd):
                wfc1 = np.memmap(os.path.join(m.wfcdirectory, f"k0{neighbor}.wfc"), mode='r', dtype=np.complex128,
                                offset=band1 * m.nr * np.dtype(np.complex128).itemsize, shape=(m.nr,)).conj()
