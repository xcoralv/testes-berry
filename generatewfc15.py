...
class WfcGenerator:
    ...
    async def run(self):
        ...
        if isinstance(self.nk_points, range):
            ...
            await self._wfck2r(self.nk_points, 0, m.nbnd)
    ...
    async def _wfck2r(self, nk_points: int, initial_band: int, number_of_bands: int):
        ...
        if m.noncolin:
            ...
            # Create ThreadPoolExecutor with a number of workers equal to m.nks
            with ProcessPoolExecutor(max_workers=m.nks) as executor:
                loop = asyncio.get_running_loop()

                # Schedule tasks for all nk_points
                tasks = [
            ...
            """Process all k-points concurrently."""
#            tasks = []
 #           for nk_point in self.nk_points:
  #              tasks.append(self.aux(nk_point))
   #         await asyncio.gather(*tasks)

            

    
    def aux(self, nk_point: int):
        ...
        outfile = os.path.join(m.wfcdirectory, f"k0{nk_point}")
