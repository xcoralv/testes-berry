...
class WfcGenerator:
    ...
    def _wfck2r(self, nk_point: int, initial_band: int, number_of_bands: int):
        ...
        if m.noncolin:
            ...
            outfile = os.path.join(m.wfcdirectory, f"k0{nk_point}.npz") #savez doesn't like to save with other extensions that not .npz

            portion_dict = {}
            
            # Save the portions into a single .npz file
            for i in range(m.nbnd):
                # Extract the portion of the array
                portion = psifinal[i * m.nr : (i + 1) * m.nr]
                ...
                portion_dict[key_name] = portion

            # Save the entire dictionary into the .npz file
            with open(outfile, 'wb') as fich:
                np.savez(fich, **portion_dict)
