...
class WfcGenerator:
    ...
    def run(self):
        ...
        if isinstance(self.nk_points, range):
            ...
            # Open HDF5 file once, write incrementally
            with h5py.File(self.outfile.replace(".npy", ".h5"), "w") as h5f:

                # store metadata as attributes
                h5f.attrs["nks"] = m.nks
                h5f.attrs["nbnd"] = m.nbnd
                h5f.attrs["nr"] = m.nr
                ...
                # Write each result incrementally into HDF5
                for portion_dict in results:
                    for key, data in portion_dict.items():
